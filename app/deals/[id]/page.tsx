...
export default function DealPage() {
  ...
  async function decide(decision: HumanDecision, confirmed = false) {
    ...
    try {
      ...
      if (data.challenged) {
        setLocalChallenge({ decision, message: data.message });
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  ${shownDecision && s.decisionDocument
      ? `<div class="decision"><b>${shownDecision.toUpperCase()} — generated document</b><p style="white-space:pre-wrap">${escapeHtml(s.decisionDocument)}</p></div>`
                ...
                <>
                  ...
                  {s.decisionDocument ? (
                    <div className="mt-3 rounded-lg bg-neutral-800/40 p-3">
                      <p className="text-[11px] uppercase tracking-widest text-neutral-500 mb-1">Generated document</p>
                      <div className="text-sm text-neutral-300 whitespace-pre-wrap leading-relaxed">{s.decisionDocument}</div>
                    </div>
                  ) : ns ? (
                    <div className="mt-3 rounded-lg bg-neutral-800/40 p-3">
