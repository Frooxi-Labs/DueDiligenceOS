    ...
    <div className="h-full flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div ref={scrollRef} className="flex-1 overflow-auto df-scroll px-6 pb-2 space-y-3">
          ...
          {/* Deal memo — inline in the conversation */}
          {s.recommendation && (
            <div className="fade-up rounded-xl border border-neutral-700 bg-neutral-900 overflow-hidden my-2">
              <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800" style={{ background: s.signal === 'green' ? '#0e2a1a' : s.signal === 'yellow' ? '#2a230e' : '#2a0e0e' }}>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right"><p className="text-[10px] uppercase tracking-widest text-neutral-500">Composite risk</p><p className="text-lg font-semibold">{s.compositeScore ?? '—'}<span className="text-xs text-neutral-500">/100</span></p></div>
              ...
              <div className="p-4 space-y-3">
                ...
                {s.conditions && s.conditions.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1">{s.conditions.map((c, i) => <li key={i} className="flex gap-2 text-sm text-neutral-300"><span className="text-neutral-600">☐</span> {c}</li>)}</ul>
                  </div>
                )}
              </div>
              <div className="px-4 py-3 border-t border-neutral-800">
                {shownDecision ? (
                  <>
                    <p className="text-sm"><span className="text-neutral-500">Reviewer decision:</span> <span className="text-white font-semibold capitalize">{shownDecision}</span><span className="text-neutral-600"> · stamped {new Date().toLocaleString()}</span></p>
                    {s.decisionDocument ? (
                      <div className="mt-3 rounded-lg bg-neutral-800/40 p-3"><p className="text-[11px] uppercase tracking-widest text-neutral-500 mb-1">Generated document</p><div className="text-sm text-neutral-300 whitespace-pre-wrap leading-relaxed">{s.decisionDocument}</div></div>
                    ) : ns ? (
